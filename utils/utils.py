...
def read_nuscenes_yaml():
    with open('/home/fbalde/clean_Semcity/dataset_c/nuscenes.yaml', 'r') as stream:
        nuscenesyaml = yaml.safe_load(stream)
        # there is no learing map in the yaml file, we need to create it
        learning_map = None
...
def dict_to_namespace(d):
    ...
    Returns:
        argparse.Namespace: Namespace object with the same structure as the dictionary
    """
...
def load_config_from_yaml(config_yaml_path, return_namespace=True):
    """
    Load and return the config/args from a YAML config file.

    Args:
        config_yaml_path: Path to a YAML config file
        return_namespace: If True, convert the dict to a Namespace object. If False, return as dict.

    Returns:
        argparse.Namespace or dict: The config loaded from the YAML file, as Namespace (default) or dict

    Raises:
        FileNotFoundError: If the config file doesn't exist
        ValueError: If the path is not a YAML file
    """
    if config_yaml_path is None:
        return None

    if not (config_yaml_path.endswith('.yaml') or config_yaml_path.endswith('.yml')):
        raise ValueError(f"Expected a YAML config file, got: {config_yaml_path}")
    ...
    # Convert to namespace if requested
    if return_namespace:

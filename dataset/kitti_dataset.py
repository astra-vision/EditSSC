...
class SemKITTI(data.Dataset):
    def __init__(self, args, imageset='train', get_query=True, folder = 'voxels',augment=False):
        ...
        if imageset == 'train':
            ...
            if self.args.no_weights_ce :
                self.weights = torch.Tensor(np.ones(20) * 3).to(self.device)
            else:
                compl_labelweights = complt_num_per_class / np.sum(complt_num_per_class)
                self.weights = torch.Tensor(np.power(np.amax(compl_labelweights) / compl_labelweights, 1 / 3.0)).to(self.device)
    ...
    def __getitem__(self, index):
        ...
        path = self.scans[index]["volume_path"]
        if self.imageset == 'test':
            voxel_label = np.zeros([256, 256, 32], dtype=int).reshape((-1, 1))
        else:
            voxel_label = np.fromfile(path, dtype=np.uint16).reshape((-1, 1)) 
        ...
        if self.get_query :
            if self.imageset == 'train' and self.augment :
...
def get_query_all(voxel_label, grid_size=(256, 256, 32), voxel_indices=None, max_points=None):
    """
    Sélectionne les points de la grille, avec leurs labels,
    et calcule leurs coordonnées normalisées dans [-1, 1].

    Args:
        max_points: if set, randomly subsample to at most this many points.
                    Keeps the grid balanced (no class bias) via uniform random sampling.
    ...
    query[:, 2] = 2 * coords[:, 2] / float(grid_size[2] - 1) - 1

    return query, labels, coords

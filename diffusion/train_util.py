...
from diffusion.loss_ae_computation import Decoder_loss
from diffusion.unet_triplane import pad_small_triplanes_with_noise
...
class TrainLoop:
    ...
    def _sample_and_visualize(self):
        ...
        # Process and visualize conditioning if using small cond triplane (for debugging)
        processed_cond = None
        ...
        with th.no_grad():
            if self.ssc_refine:
                if self.args.large_noise_trick:
                    ...
                    noise = self.ddp_model(m_t, large_T, cond['H'], cond['W'], cond['D'], cond['y'], data=data).to(dist_util.dev())
                else :
                    noise = None

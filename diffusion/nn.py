...
def mask_img(img, cond, mode, overlap, H=[128]):
    ...
    if th.is_tensor(mode):
        # mode is a binary mask: 1 for inpaint (use img), 0 for keep GT (use cond)
        return cond * (1 - mode) + img * mode
    if type(mode) == tuple:
...
def decompose_featmaps(composed_map, tri_size=(128,128,16) , transpose=True,condition=False):
    H, W, D = tri_size

...
from diffusion_c.nn import mean_flat, mask_img, decompose_featmaps
from diffusion_c.losses import normal_kl, discretized_gaussian_log_likelihood
from diffusion_c.scheduler import get_schedule_jump
from diffusion_c.loss_ae_computation import Decoder_loss
...
class GaussianDiffusion:
    ...
    def __init__(
        ...
        *,
        ...
        self.velocity_loss = getattr(args, 'velocity_loss', False)
        self.kl_dist_loss = getattr(args, 'kl_dist_loss', False)
        self.kl_dist_loss_weight = getattr(args, 'kl_dist_loss_weight', 0.2)
        self.weighted_mse_loss = getattr(args, 'weighted_mse_loss', False)
        self.weighted_mse_weight = getattr(args, 'weighted_mse_weight', 1.0)
        self.args = args

        # Check if using 3D latent architecture
        # Priority: explicit use_3d_latent parameter > ae_architecture inference
        if hasattr(args, 'use_3d_latent') and args.use_3d_latent is not None:
            self.use_3d_latent = args.use_3d_latent
        else:
            # Fallback: infer from ae_architecture
            ae_architecture = getattr(args, 'ae_architecture', 'triplane')
            self.use_3d_latent = ae_architecture in ['3d_latent', 'sparse_3d', 'vq_lidar', 'sparse_lidar']
        if self.args.decoder_loss or self.kl_dist_loss or self.weighted_mse_loss:
            self.decoder_loss=Decoder_loss(args=self.args)
    ...
    def p_mean_variance(
        ...
        model_output = model(x, self._scale_timesteps(t), model_kwargs['H'], model_kwargs['W'], model_kwargs['D'], model_kwargs['y'], data=data)
    ...
    def p_sample(
        ...
        if self.args.cfg:
            ...
            new_kwargs['y']=th.zeros_like(model_kwargs['y'])
            # print("model_kwargs", model_kwargs['y'])
            # print("new_kwargs", new_kwargs['y'])
            ...
            samples_uncond= out_uncond["mean"] + nonzero_mask * th.exp(0.5 * out_uncond["log_variance"]) * noise

            sample = samples_uncond + (1)*(sample - samples_uncond)
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.ssc_refine :
            if self.args.large_noise_trick:

                with th.no_grad():
                    large_T = th.tensor([self.num_timesteps-1] * x_start.shape[0], device=x_start.device)
                    m_t = self.q_sample(x_start, large_T)
                    m_1 = model(m_t, large_T, model_kwargs['H'], model_kwargs['W'], model_kwargs['D'], model_kwargs['y'], data=data)
        ...
        model_output = model(x_t, self._scale_timesteps(t), model_kwargs['H'], model_kwargs['W'], model_kwargs['D'], model_kwargs['y'], data=data)
        ...
        if self.velocity_loss:
            sqrt_alpha_t = _extract_into_tensor(self.sqrt_alphas_cumprod, t, x_t.shape)
            sqrt_one_minus_alpha_t = _extract_into_tensor(self.sqrt_one_minus_alphas_cumprod, t, x_t.shape)
            # v = sqrt(alpha)*eps - sqrt(1-alpha)*x0
            target = sqrt_alpha_t * noise - sqrt_one_minus_alpha_t * target
            # For model_output, we assume it's x0 (since velocity_loss is for x0)
            # v_pred = (sqrt(alpha)*x_t - x0_pred) / sqrt(1-alpha)
            model_output = (sqrt_alpha_t * x_t - model_output) / sqrt_one_minus_alpha_t

        H, W, D = model_kwargs["H"], model_kwargs["W"], model_kwargs["D"]
        trisize = (H[0], W[0], D[0])

      
        # Path 2 & 3: Triplane models (either standard triplane or Conv3DAwareUNet)
        # Both paths decompose triplanes, but in different ways
        if self.args.training_sd_latent:
            ...
            # Path 2: Standard Triplane - decomposes via decompose_featmaps
            ...
            if self.triplane_loss_type == 'l1':
                ...
                if self.args.use_bev_unet or self.args.use_stable_diffusion_bev:
                    terms["l1_xz"]=0
                    terms["l1_yz"]=0
                else:
                ...
                if self.weighted_mse_loss:
                    terms["l2_xy"] = self.decoder_loss.compute_weighted_mse_loss(model_output, target)
                else:
                ...
                if self.args.decoder_loss:
                    ce_loss,lovasz_loss=self.decoder_loss.compute_ae_loss(model_output,target)
                    terms["regul"]=ce_loss+lovasz_loss

                if self.kl_dist_loss:
                    terms["kl_dist_loss"] = self.decoder_loss.compute_kl_dist_loss(model_output)

                if "vb" in terms:
                    terms["loss"] = terms["l2_xy"] + terms["l2_xz"] + terms["l2_yz"] + terms["vb"]
                    # terms["loss"] += (terms["vis_xy"] + terms["vis_xz"] + terms["vis_yz"]) / 3   #/2
                    if self.args.decoder_loss:
                        terms["loss"] += terms["regul"]/5
                    
                    if self.kl_dist_loss:
                        terms["loss"] += terms["kl_dist_loss"] * self.kl_dist_loss_weight

...
class GaussianDiffusion:
    ...
    def p_sample_loop_scene_repaint_sequential(
        self,
        model,
        shape,
        cond,
        ...
        """
        Repaint séquentiel : dénoisage sur tous les timesteps (T -> 0) sans sauts.
        À chaque pas : on ré-injecte la zone non masquée (condition bruitée) puis p_sample.

        Si ``repaint_steps`` est un entier, on n'applique cette ré-injection que pour les
        ``repaint_steps`` premiers pas (en partant de t = T-1). Ensuite, seul ``p_sample``
        est appliqué (dénoisage libre, sans remplacer par la condition bruitée).
        ``None`` (défaut) : même comportement qu'avant, repaint à chaque pas.
        """
        if device is None:
            device = next(model.parameters()).device
        assert isinstance(shape, (tuple, list))
        ...
        indices = list(range(self.num_timesteps))[::-1]  # T-1, T-2, ..., 1, 0
        n_repaint = len(indices) if repaint_steps is None else int(repaint_steps)
        if n_repaint < 0:
            raise ValueError("repaint_steps must be non-negative")
        n_repaint = min(n_repaint, len(indices))

        with th.no_grad():
            for step_idx, t in enumerate(indices):
                t_t = th.tensor([t] * shape[0], device=device)
                if step_idx < n_repaint:
                    t_cond = self.q_sample(mask_cond, t_t)
                    image_after_step = mask_img(
                        image_after_step, t_cond, mode, overlap, H=model_kwargs['H']
                    )
                out = self.p_sample(
                    model,
                    image_after_step,
    ...
    def training_losses(self, model, x_start, t, model_kwargs=None, noise=None):
        ...
        if self.triplane_loss_type == 'l1':
            ...
            if self.args.use_stable_diffusion_bev:
                terms["l1_xz"] = 0
                terms["l1_yz"] = 0
            else:

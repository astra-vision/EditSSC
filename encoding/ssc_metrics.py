...
class SSCMetrics:
    ...
    def one_stats(self, x, y):
        ...
        idxs = tuple(np.stack((x_row, y_row), axis=0)) # pour avoir des tuples de classes pred, true classe
        conf_matrix = np.zeros((self.n_classes, self.n_classes), dtype=np.int64)
        np.add.at(conf_matrix, idxs, 1) # on rajoute des 1 dans chaque tuple de la conf matrix
        conf_matrix[:, self.ignore] = 0 # 0 sur la colonne à ignorer
        tp = np.diag(conf_matrix)   
        fp = conf_matrix.sum(axis=1) - tp
        fn = conf_matrix.sum(axis=0) - tp
        intersection = tp  # de taille 20 aussi donc inter par classe
        union = tp + fp + fn + 1e-15 # de taille 20 donc union pour chaque classe
        n = len(np.unique(y)) - 1
        miou = (intersection[1:] / union[1:]).sum()/n *100 # moyenne des iou par classe
        #miou = (intersection / union).sum()/n *100
        all_miou = (intersection / union).sum()/(n+1) *100
        iou = (np.sum(conf_matrix[1:, 1:])) / (np.sum(conf_matrix) - conf_matrix[0, 0] + 1e-8) * 100 # iou entre empty et occupied TP = occupied
        return iou, miou, all_miou # du batch en question pas juste pour une scène
    
    def addBatch(self, x, y):  # x=preds, y=targets calcule l'iou pour le batch en question!!
        # sizes should be matching
        x_row = x.reshape(-1)  # de-batchify
        y_row = y.reshape(-1)  # de-batchify
    ...
    def get_confusion(self):
        return self.conf_matrix.copy()  # matrice de confusion de tout le dataset